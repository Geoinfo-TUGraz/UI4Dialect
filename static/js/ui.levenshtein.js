/** Creates ajax request for loading levenshtein data from database and create a layer from it */
function ajaxRequestForLevenshtein(inputValue, levenshteinValue, levenshteinSelect) {
	$.ajax({
		url: '/searchLevenshtein',
		type: 'POST',
...
/** Click on Levenshtein search button */
$(document).on('click', '#btn-levenshtein', function (e) {
	...
	if (!$("#input-levenshtein").val()) {
		...
		ajaxRequestForLevenshtein(inputValue, levenshteinValue, levenshteinSelect);
	}
});
...
/** Restores the levenshtein data when layer is re-enabled in layerControl */
